...
\begin{tikzpicture}[scale=1.1, transform shape]
    % --- NODI DELL'ALBERO SINTATTICO (Posizionamento) ---
    \node (T) at (0, 0) {$T$};
    ...
    % Figli di F1
    \node (digit1) at (-4, -4) {\textbf{digit}};
    
    % Figli di T' (primo livello)
    \node (mul) at (-0.5, -4) {$*$};
    \node (F2) at (2, -4) {$F$};
    \node (Tp2) at (5, -4) {$T'$};
    
    % Figli di F2
    \node (digit2) at (2, -6) {\textbf{digit}};
    
    % Figli di T' (secondo livello)
    \node (eps) at (5, -6) {$\epsilon$};

    % --- LINEE DELL'ALBERO (Tratteggiate) ---
    \draw[dotted, thick] (T) -- (F1);
    \draw[dotted, thick] (T) -- (Tp1);
    \draw[dotted, thick] (F1) -- (digit1);
    \draw[dotted, thick] (Tp1) -- (mul);
    \draw[dotted, thick] (Tp1) -- (F2);
    \draw[dotted, thick] (Tp1) -- (Tp2);
    \draw[dotted, thick] (F2) -- (digit2);
    \draw[dotted, thick] (Tp2) -- (eps);

    % --- ATTRIBUTI SEMANTICI (Nodi con numeri) ---
    
    % Attributi T
    \node[right=0.1cm of T, font=\small] (val9) {9 \textit{val}};
    
    % Attributi F1 e digit1
    \node[right=0.1cm of F1, font=\small] (val3) {3 \textit{val}};
    \node[right=0.1cm of digit1, font=\small] (lex1) {1 \textit{lexval}};
    
    % Attributi T' (Livello 1)
    \node[left=0.05cm of Tp1, font=\small] (inh5) {\textit{inh} 5};
    \node[right=0.05cm of Tp1, font=\small] (syn8) {8 \textit{syn}};
    
    % Attributi F2 e digit2
    \node[right=0.1cm of F2, font=\small] (val4) {4 \textit{val}};
    \node[right=0.1cm of digit2, font=\small] (lex2) {2 \textit{lexval}};
    
    % Attributi T' (Livello 2)
    \node[left=0.05cm of Tp2, font=\small] (inh6) {\textit{inh} 6};
    \node[right=0.05cm of Tp2, font=\small] (syn7) {7 \textit{syn}};

    % --- FRECCE DELLE DIPENDENZE ---
    
    % 1 lexval -> 3 val (Verticale)
    \draw[->, >=latex] (lex1) -- (val3);
    
    % 3 val -> inh 5 (Curva sopra)
    \draw[->, >=latex] (val3) to[out=25, in=165] (inh5);
    
    % 2 lexval -> 4 val (Verticale)
    \draw[->, >=latex] (lex2) -- (val4);
    
    % 4 val -> inh 6 (Curva sopra)
    \draw[->, >=latex] (val4) to[out=25, in=165] (inh6);
    
    % inh 5 -> inh 6 (Diagonale)
    \draw[->, >=latex] (inh5) -- (inh6);
    
    % inh 6 -> syn 7 (Curva sotto a cappio)
    \draw[->, >=latex] (inh6) to[out=-80, in=-100, looseness=2] (syn7);
    
    % syn 7 -> syn 8 (Diagonale risalita)
    \draw[->, >=latex] (syn7) -- (syn8);
    
    % syn 8 -> 9 val (Diagonale risalita)
    \draw[->, >=latex] (syn8) -- (val9);
